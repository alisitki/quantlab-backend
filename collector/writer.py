...
class ParquetWriter:
    ...
    def _verify_parquet(self, filepath: str) -> tuple:
        """
        Verify parquet file is readable after write.
        Catches Snappy/Thrift corruption before committing the file.
    ...
    def _write_to_spool(self, table: pa.Table, exchange: str, stream: str, symbol: str) -> bool:
        ...
        try:
            ...
            # 3. Verify file is readable (catch Snappy corruption)
            valid, verify_msg = self._verify_parquet(tmp_path)
            if not valid:
                print(f"[Writer] POST-WRITE VERIFICATION FAILED: {exchange}/{stream}/{symbol} - {verify_msg}")
