...
function deriveConfidenceRatio(tStat, eventCount) {
  return Math.min(Math.abs(tStat) / 2.0, eventCount / 200.0);
}
...
function validateConfig(rawConfig) {
  ...
  if (familyId !== 'return_reversal_v1') {
    throw new Error('RETURN_REVERSAL_V1_CONFIG_ERROR: family_id must be return_reversal_v1');
  }

  const bindingMode = String(config.binding_mode || '').trim();
  if (bindingMode !== 'PAPER_DIRECTIONAL_V1') {
    throw new Error('RETURN_REVERSAL_V1_CONFIG_ERROR: binding_mode must be PAPER_DIRECTIONAL_V1');
  }

  const stream = String(config.stream || '').trim().toLowerCase();
  if (stream !== 'trade') {
    throw new Error('RETURN_REVERSAL_V1_CONFIG_ERROR: stream must be trade');
  }
  const exchange = String(config.exchange || '').trim().toLowerCase();
  if (!exchange) {
    throw new Error('RETURN_REVERSAL_V1_CONFIG_ERROR: exchange required');
  }
  ...
  const symbols = Array.isArray(config.symbols)
    ...
    : [];
  if (symbols.length !== 1) {
    throw new Error('RETURN_REVERSAL_V1_CONFIG_ERROR: exactly one symbol required');
  }

  const orderQty = toPositiveNumber(config.orderQty);
  if (orderQty === null) {
    throw new Error('RETURN_REVERSAL_V1_CONFIG_ERROR: positive orderQty required');
  }
  ...
  if (!params) {
    throw new Error('RETURN_REVERSAL_V1_CONFIG_ERROR: params object required');
  }
  if (!selectedCell) {
    throw new Error('RETURN_REVERSAL_V1_CONFIG_ERROR: selected_cell object required');
  }

  const deltaMs = toPositiveInt(selectedCell.delta_ms);
  const hMs = toPositiveInt(selectedCell.h_ms);
  if (!deltaMs || !hMs) {
    throw new Error('RETURN_REVERSAL_V1_CONFIG_ERROR: selected_cell delta_ms and h_ms must be positive integers');
  }

  const selectedSymbol = normalizeSymbol(selectedCell.symbol);
  if (selectedSymbol !== symbols[0]) {
    throw new Error('RETURN_REVERSAL_V1_CONFIG_ERROR: selected_cell symbol mismatch');
  }
  if (String(selectedCell.stream || '').trim().toLowerCase() !== stream) {
    throw new Error('RETURN_REVERSAL_V1_CONFIG_ERROR: selected_cell stream mismatch');
  }
  if (String(selectedCell.exchange || '').trim().toLowerCase() !== exchange) {
    throw new Error('RETURN_REVERSAL_V1_CONFIG_ERROR: selected_cell exchange mismatch');
  }
  ...
  if (eventCount === null || meanProduct === null || tStat === null) {
    throw new Error('RETURN_REVERSAL_V1_CONFIG_ERROR: selected_cell event_count/mean_product/t_stat required');
  }
  if (!(meanProduct < 0) || !(tStat <= -2)) {
    throw new Error('RETURN_REVERSAL_V1_CONFIG_ERROR: selected_cell must satisfy return reversal pass bar');
  }
...
export class ReturnReversalV1Strategy {
  constructor(config = {}) {
  ...
  async onEvent(event, ctx) {
    ...
    if (this.state.order_event_count === 1) {
      ctx.logger.info(
        `[ReturnReversalV1Strategy] first_order action=${action} side=${orderIntent.side} qty=${orderIntent.qty} symbol=${this.config.symbol}`
      );
    }
  }
  ...
  async onFinalize(ctx) {
    ...
    ctx.logger.info(
      `[ReturnReversalV1Strategy] finalize matched_trade_events=${this.state.matched_trade_events} signal_event_count=${this.state.signal_event_count} order_event_count=${this.state.order_event_count}`
    );
  }
...
export default ReturnReversalV1Strategy;
