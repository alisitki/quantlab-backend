...
SCRIPT = REPO / "tools" / "hypotheses" / "spread_reversion_v1.py"
RUNNER = REPO / "tools" / "run-multi-hypothesis.js"


class SpreadReversionV1ContractTests(unittest.TestCase):
    def setUp(self) -> None:
        token = uuid4().hex[:10]
        self.exchange = f"usr{token}"
        self.symbol = f"srv1{token}usdt"
        self.created_paths = []
        self.tmpdir = Path(tempfile.mkdtemp(prefix="srv1_contract_"))
    ...
    def _write_bbo_day(self, day: str, ts, seq, bid, ask) -> None:
        out = (
            REPO
            / "data"
            / "curated"
    ...
    def test_golden_header_and_stable_row_order(self) -> None:
        ...
        results = self._run_family(start="20990301", end="20990302", delta_ms="2,1", h_ms="1")
    ...
    def test_t_stat_zero_when_sample_size_below_two(self) -> None:
        ...
        results = self._run_family(start="20990303", end="20990303", delta_ms="2", h_ms="2")
        with results.open("r", encoding="utf-8", newline="") as f:
    ...
    def test_runner_contract_contains_spread_family_and_args(self) -> None:
        text = RUNNER.read_text(encoding="utf-8")
        self.assertIn("parseFamilySpreadReversion", text)
        self.assertIn("parseSpreadReversionResults", text)
        self.assertIn("--srDeltaMsList", text)
        self.assertIn("--srHMsList", text)
        self.assertIn("spread_reversion_v1", text)
