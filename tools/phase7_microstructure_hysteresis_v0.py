...
def compose_strategy_id(base_row: dict) -> str:
    ...
    return (
        ...
        f"__xt{threshold_token(EXIT_THRESHOLD)}"
    )
...
def prepare_run(output_dir: Path) -> PreparedRun:
    ...
    watchlist = {
        "schema_version": "phase7_microstructure_hysteresis_watchlist_v0",
        "generated_ts_utc": iso_utc_now(),
        "governance": {
            "surface_role": "TASK_LOCAL_LINKUSDT_HYSTERESIS_VALIDATION",
            "not_authoritative_for": ["global shadow watchlist", "ranking", "promotion"],
            "notes": [
                "linkusdt-only hysteresis validation",
                "global shadow_watchlist is not read or written",
                "single lever only: entry threshold 0.2, exit threshold 0.1",
            ],
        },
        "selected_count": 1,
        "items": [{
            ...
            "exchange": "bybit",
            "symbols": ["linkusdt"],
            "decision_tier": "PHASE7_HYSTERESIS_VALIDATION",
    ...
    config = {
        ...
        "source_decision_tier": "PHASE7_HYSTERESIS_VALIDATION",
        "exchange": "bybit",
        "stream": "trade",
        "symbols": ["linkusdt"],
        "source_family_report_path": "",
        "window": "hysteresis_validation",
        "orderQty": 1,
        "params": {
...
def run_batch(prepared: PreparedRun, run_max_duration_sec: int, per_run_timeout_sec: int, heartbeat_ms: int, progress_interval_sec: int) -> int:
    ...
    print(
        "PHASE7_MICROSTRUCTURE_HYSTERESIS_STARTED "
    ...
    while True:
        try:
            ...
            print(
                "PHASE7_MICROSTRUCTURE_HYSTERESIS_PROGRESS "
                f"elapsed_sec={elapsed} still_running=1",
                flush=True,
            )
            next_progress += progress_interval_sec
    print(
        "PHASE7_MICROSTRUCTURE_HYSTERESIS_DONE "
...
def analyze_run(prepared: PreparedRun, result_json: Path, report_md: Path, run_mode: str) -> dict:
    ...
    baseline_action = action_metrics_from_paper_ledger(Path(baseline["source_paths"]["shadow_futures_paper_ledger_v1_json"]), BASE_STRATEGY_ID)
    after_action = action_metrics_from_paper_ledger(prepared.output_dir / "shadow_state/shadow_futures_paper_ledger_v1.json", prepared.strategy_id)

    base = {
        "fill_count": baseline["trade_quality"]["fill_count"],
        "decision_count": baseline["trade_quality"]["decision_count"],
        ...
        "open_count": baseline_action["open_count"],
        "close_count": baseline_action["close_count"],
        "reversal_count": baseline_action["reversal_count"],
        "reversal_per_1k_events": ratio(baseline_action["reversal_count"] * 1000, baseline["efficiency_metrics"]["processed_event_count"]),
    }
    after = {
    ...
    reversal_reduction_ratio = ratio(base["reversal_count"] - after["reversal_count"], base["reversal_count"])
    ...
    if after["verify_soft_live_pass"] is not True or (after["processed_event_count"] or 0) <= 0:
        ...
        (after["reversal_per_1k_events"] or 0) < (base["reversal_per_1k_events"] or 0)
        and (gross_per_fill_change_ratio or 0) > 0
        and (gross_preserved_ratio or 0) >= 0.5
    ):
        verdict = "IMPROVED"
        reason = "reversal churn fell and gross pnl per fill improved without collapsing gross edge"
    else:
        verdict = "NO_IMPROVEMENT"
        reason = "critical churn/pnl-per-trade target not met"

    payload = {
        "schema_version": "phase7_microstructure_hysteresis_v0",
        "generated_ts_utc": iso_utc_now(),
        "run_mode": run_mode,
        ...
        "lever": {
            "selected": "HYSTERESIS",
            "implementation": "entry threshold 0.2, exit threshold 0.1, reverse only on strong opposite signal",
        ...
        "comparison": {
            ...
            "reversal_reduction_ratio": reversal_reduction_ratio,
        },
        "verdict": verdict,
    ...
    report_md.write_text(
        ...
        f"- baseline reversals/1k events: `{base['reversal_per_1k_events']}` after reversals/1k events: `{after['reversal_per_1k_events']}`\n"
...
def main() -> int:
    ...
    print("PHASE7_MICROSTRUCTURE_HYSTERESIS_COMPLETE", flush=True)
