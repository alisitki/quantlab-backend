...
def normalize_funding_events(run_summary: dict[str, Any]) -> list[dict[str, Any]]:
    raw = run_summary.get("funding_events")
    if not isinstance(raw, list):
        return []

    normalized: list[dict[str, Any]] = []
    for index, raw_event in enumerate(raw, start=1):
...
def normalize_mark_price_events(run_summary: dict[str, Any]) -> list[dict[str, Any]]:
    raw = run_summary.get("mark_price_events")
    if not isinstance(raw, list):
        return []
...
def normalize_execution_event(action: Any, metadata: Any, *, event_seq: int) -> dict[str, Any] | None:
    ...
    try:
        ...
        fill_price = None
    try:
        fill_fee = float(metadata.get("fill_fee"))
    except (TypeError, ValueError):
        fill_fee = None
    try:
    ...
    *,
    ...
    return {
        ...
        **selected,
        ...
        "funding_events": normalize_funding_events(run_summary),
        "mark_price_events": normalize_mark_price_events(run_summary),
